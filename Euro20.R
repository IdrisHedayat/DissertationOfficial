...
setwd("~/Dropbox/Blog/Football_Kaggle/")
...
# Schedule for WC2022
schedule=read.csv("fifa-world-cup-2022-schedule.csv") %>% as_tibble() %>% 
   mutate(
      ...
      Home.Team=case_when(
        Home.Team=="USA"~"United States",
        Home.Team=="Korea Republic"~"South Korea",
        TRUE~Home.Team
      ),
      Away.Team=case_when(
        Away.Team=="USA"~"United States",
        Away.Team=="Korea Republic"~"South Korea",
        TRUE~Away.Team
      ),
      Location="Qatar",
...
footie=read.csv("~/Dropbox/Blog/Football_Kaggle/International_history_2022.csv") %>% as_tibble() %>% 
...
ranking=read.csv("FIFA_ranking_2022.csv") %>% as_tibble() %>% 
  mutate(
