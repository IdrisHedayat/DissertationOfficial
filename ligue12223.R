...
# Long format
liguesched=(
  liguesched %>% select(ID_game,date,home_team,home_score,tournament,Location,Round.Number) %>% 
    rename(Team=home_team,Goal=home_score) 
) %>% 
  bind_rows(
    liguesched %>% select(ID_game,date,away_team,away_score,tournament,Location,Round.Number) %>% 
...
liguefootie=liguesched

# creates opponent variable 

# liguefootie = liguefootie %>% 
#   mutate(
...
liguefootie=liguefootie %>% group_by(ID_game) %>% 
  mutate(
